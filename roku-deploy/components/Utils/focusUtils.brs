...
'Sets focus on '
function applyFocus(obj as Object, focusState = true as Boolean, log = "" as String, isLocal = false as boolean )

  if (isValid(obj))

    scope = getScene()
    if ( isLocal ) then scope = m

    obj.setFocus(focusState)
    scope.currentFocusItem = obj
...
' @param - Boolean scope of focusHandler
' @return - Boolean returns true if focus has been set to a component, false otherwise
function componentFocusHandler( key, focusMap, currentFocusedComponent, isLocal = false ) as Boolean
  ...
  if ( isValid( componentFocusMap ) )
    ...
    if isValid( nextFocusedItemId )
        nextFocusedItem = scene.findNode( nextFocusedItemId )
        applyFocus( nextFocusedItem, true, m.top.id + " componentFocusHandler() - componentFocusHandler.brs ", isLocal )
        focusState = true
    else
        focusState = false
    end if
...
' @param object scene
' @param boolean is scope local
' @return node current node that has focus
function getCurrentFocusItem( scene = getScene(), isLocal = false )
  scope = scene

  if ( isLocal ) then scope = m
  return scope.currentFocusItem
