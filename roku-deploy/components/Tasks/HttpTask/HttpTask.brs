...
function createResponseModel( response as Object ) as Object
  ...
  #if enable_http_logs
  logHttpResponse( response.request.url, response.error, response.code, response.data, response.headers )
#end if
...
' @param object data
' @param object headers
function logHttpResponse( url as String, errorStatus, code, data, headers )
  ...
  print " Code: "; code
  if ( isValid( headers ) ) then print " Headers: "; headers
  print " Data: "; data
  print ""
  print "*****************************************"
